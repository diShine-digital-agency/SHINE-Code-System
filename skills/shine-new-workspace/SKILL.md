...
- `--path` — Target directory. Defaults to `~/shine-workspaces/<name>`
...
<objective>
Create a physical workspace directory containing copies of specified git repos (as worktrees or clones) with an independent `.planning/` directory for isolated SHINE sessions.

**Use cases:**
- Multi-repo orchestration: work on a subset of repos in parallel with isolated SHINE state
...
**After this command:** `cd` into the workspace and run `/shine-new-project` to initialize SHINE.
</objective>

<execution_context>
@~/.claude/shine/workflows/new-workspace.md
@~/.claude/shine/references/ui-brand.md
</execution_context>

<process>
Execute the new-workspace workflow from @~/.claude/shine/workflows/new-workspace.md end-to-end.
