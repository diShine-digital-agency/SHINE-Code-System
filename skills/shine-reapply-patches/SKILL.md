---
name: shine-reapply-patches
description: "Reapply local modifications after a SHINE update"
---


<purpose>
After a SHINE update wipes and reinstalls files, this command merges user's previously saved local modifications back into the new version. Uses three-way comparison (pristine baseline, user-modified backup, newly installed version) to reliably distinguish user customizations from version drift.

**Critical invariant:** Every file in `shine-local-patches/` was backed up because the installer's hash comparison detected it was modified. The workflow must NEVER conclude "no custom content" for any backed-up file — that is a logical contradiction. When in doubt, classify as CONFLICT requiring user review, not SKIP.
</purpose>
...
if [ -n "$KILO_CONFIG_DIR" ]; then
  candidate="$(expand_home "$KILO_CONFIG_DIR")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$KILO_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$KILO_CONFIG")")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/kilo/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$OPENCODE_CONFIG_DIR")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$OPENCODE_CONFIG" ]; then
  candidate="$(dirname "$(expand_home "$OPENCODE_CONFIG")")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
elif [ -z "$PATCHES_DIR" ] && [ -n "$XDG_CONFIG_HOME" ]; then
  candidate="$(expand_home "$XDG_CONFIG_HOME")/opencode/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$GEMINI_CONFIG_DIR" ]; then
  candidate="$(expand_home "$GEMINI_CONFIG_DIR")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CODEX_HOME" ]; then
  candidate="$(expand_home "$CODEX_HOME")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ] && [ -n "$CLAUDE_CONFIG_DIR" ]; then
  candidate="$(expand_home "$CLAUDE_CONFIG_DIR")/shine-local-patches"
  if [ -d "$candidate" ]; then
    PATCHES_DIR="$candidate"
  fi
fi
...
if [ -z "$PATCHES_DIR" ]; then
  if [ -d "$HOME/.config/kilo/shine-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/kilo/shine-local-patches"
  elif [ -d "$HOME/.config/opencode/shine-local-patches" ]; then
    PATCHES_DIR="$HOME/.config/opencode/shine-local-patches"
  elif [ -d "$HOME/.opencode/shine-local-patches" ]; then
    PATCHES_DIR="$HOME/.opencode/shine-local-patches"
  elif [ -d "$HOME/.gemini/shine-local-patches" ]; then
    PATCHES_DIR="$HOME/.gemini/shine-local-patches"
  elif [ -d "$HOME/.codex/shine-local-patches" ]; then
    PATCHES_DIR="$HOME/.codex/shine-local-patches"
  else
    PATCHES_DIR="$HOME/.claude/shine-local-patches"
  fi
fi
# Local install fallback — check all runtime directories
if [ ! -d "$PATCHES_DIR" ]; then
  for dir in .config/kilo .kilo .config/opencode .opencode .gemini .codex .claude; do
    if [ -d "./$dir/shine-local-patches" ]; then
      PATCHES_DIR="./$dir/shine-local-patches"
      break
    fi
  done
fi
...
Local patches are automatically saved when you run /shine-update
after modifying any SHINE workflow, command, or agent files.
```
Exit.

## Step 2: Determine baseline for three-way comparison

The quality of the merge depends on having a **pristine baseline** — the original unmodified version of each file from the pre-update SHINE release. This enables three-way comparison:
- **Pristine baseline** (original SHINE file before any user edits)
- **User's version** (backed up in `shine-local-patches/`)
...
```
When `HAS_GIT=true`, use `git log` to find the commit where SHINE was originally installed (before user edits). For each file, the pristine baseline can be extracted with:
```bash
...
Check if a `shine-pristine/` directory exists alongside `shine-local-patches/`:
```bash
PRISTINE_DIR="$CONFIG_DIR/shine-pristine"
```
...
1. **Read the backed-up version** (user's modified copy from `shine-local-patches/`)
2. **Read the newly installed version** (current file after update)
3. **If available, read the pristine baseline** (from git history or `shine-pristine/`)
...
b. Identify ALL differences, then classify each as:
   - **Mechanical drift** — path substitutions (e.g. `/Users/xxx/.claude/` → `$HOME/.claude/`), variable additions (`${SHINE_WS}`, `${AGENT_SKILLS_*}`), error handling additions (`|| true`)
...
git -C "$CONFIG_DIR" log --oneline --no-merges -- "{file_path}" | grep -v "shine:update\|SHINE update\|shine-install"
```
...
Ask user:
- "Keep patch backups for reference?" → preserve `shine-local-patches/`
- "Clean up patch backups?" → remove `shine-local-patches/` directory
...
- [ ] Three-way merge used when pristine baseline available (git history or shine-pristine/)
