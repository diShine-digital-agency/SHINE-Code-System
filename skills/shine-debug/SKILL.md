...
**Orchestrator role:** Gather symptoms, spawn shine-debugger agent, handle checkpoints, spawn continuations.
...
- shine-debugger — Diagnoses and fixes issues
</available_agent_types>
...
```bash
debugger_model=$(node "$HOME/.claude/shine/bin/shine-tools.cjs" resolve-model shine-debugger --raw)
```
...
- Offer options:
  ...
  - "Plan fix" — suggest `/shine-plan-phase --gaps`
  - "Manual fix" — done

**If `## DEBUG COMPLETE` (find_and_fix mode):**
- Display root cause and fix summary
- Offer options:
  - "Plan fix" — suggest `/shine-plan-phase --gaps` if further work needed
  - "Done" — mark resolved
...
- [ ] shine-debugger spawned with context
