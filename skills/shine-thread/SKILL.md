...
```
No threads found. Create one with: /shine-thread <description>
```
</mode_list>
...
5. Commit:
   ```bash
   node "$HOME/.claude/shine/bin/shine-tools.cjs" commit "docs: create thread — ${ARGUMENTS}" --files ".planning/threads/${SLUG}.md"
   ```
...
6. Report:
   ...
   Resume anytime with: /shine-thread {slug}
   ```
</mode_create>
...
- Lighter weight than /shine-pause-work — no phase state, no plan context
- The value is in Context and Next Steps — a cold-start session can pick up immediately
- Threads can be promoted to phases or backlog items when they mature:
  /shine-add-phase or /shine-add-backlog with context from the thread
- Thread files live in .planning/threads/ — no collision with phases or other SHINE structures
</notes>
